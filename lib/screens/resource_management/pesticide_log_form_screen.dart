...
import '../../services/pesticide_management_service.dart';
...
class PesticideLogFormScreen extends StatefulWidget {
  final int cropCycleId;
  final String cropName;

  const PesticideLogFormScreen({
    super.key,
  ...
  @override
  State<PesticideLogFormScreen> createState() =>
      _PesticideLogFormScreenState();
}

class _PesticideLogFormScreenState extends State<PesticideLogFormScreen> {
  final _formKey = GlobalKey<FormState>();
  final _pesticideManagementService = PesticideManagementService();
  ...
  Future<void> _loadInventoryOptions() async {
    ...
    try {
      ...
      if (mounted) {
        // Filter inventory items by pesticide category
        final filtered = provider.items
            .where((item) =>
                item.category.toLowerCase().contains('pesticide'))
            .toList();
  ...
  Future<void> _submitForm() async {
    ...
    if (_selectedInventoryId == null) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('Please select a pesticide item'),
          backgroundColor: Colors.red,
        ),
      );
      return;
    ...
    try {
      ...
      final result = await _pesticideManagementService.createPesticideLog(
          _authToken!, data);
      ...
      if (mounted) {
        ...
        if (result['success']) {
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
              content: const Text('Pesticide application recorded successfully'),
          ...
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
              content:
                  Text(result['error'] ?? 'Failed to create pesticide log'),
              backgroundColor: Colors.red,
            ),
          );
        }
                          ...
                          children: [
                            ...
                            Padding(
                              ...
                              child: Column(
                                ...
                                children: [
                                  // Inventory selector
                                  _buildLabel('Select Pesticide Item'),
