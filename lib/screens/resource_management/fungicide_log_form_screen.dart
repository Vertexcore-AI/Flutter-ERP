...
import '../../services/fungicide_management_service.dart';
...
class FungicideLogFormScreen extends StatefulWidget {
  final int cropCycleId;
  final String cropName;

  const FungicideLogFormScreen({
    super.key,
  ...
  @override
  State<FungicideLogFormScreen> createState() =>
      _FungicideLogFormScreenState();
}

class _FungicideLogFormScreenState extends State<FungicideLogFormScreen> {
  final _formKey = GlobalKey<FormState>();
  final _fungicideManagementService = FungicideManagementService();
  ...
  Future<void> _loadInventoryOptions() async {
    ...
    try {
      ...
      if (mounted) {
        // Filter inventory items by fungicide category
        final filtered = provider.items
            .where((item) =>
                item.category.toLowerCase().contains('fungicide'))
            .toList();
  ...
  Future<void> _submitForm() async {
    ...
    if (_selectedInventoryId == null) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('Please select a fungicide item'),
          backgroundColor: Colors.red,
        ),
      );
      return;
    ...
    try {
      ...
      final result = await _fungicideManagementService.createFungicideLog(
          _authToken!, data);
      ...
      if (mounted) {
        ...
        if (result['success']) {
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
              content: const Text('Fungicide application recorded successfully'),
          ...
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
              content:
                  Text(result['error'] ?? 'Failed to create fungicide log'),
              backgroundColor: Colors.red,
            ),
          );
        }
                          ...
                          children: [
                            ...
                            Padding(
                              ...
                              child: Column(
                                ...
                                children: [
                                  // Inventory selector
                                  _buildLabel('Select Fungicide Item'),
